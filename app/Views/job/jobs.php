    ...
    <div class="col-12 grid-margin">
      ...
      <div class="text-center mt-2">
          Selected Range: <span class="badge" style="background-color: #E66136;" id="age">18 - 65</span>
                    ...
                    <div class="text-end">
                        <a href="<?= base_url('/jobview') ?>" class="btn hr-btnbg">
                            Back
                        </a>
...
<script>
    $(document).ready(function() {
        ...
        initializeAgeSlider()
        initializeSalarySlider(); // 👈 Call on page load (default range)
        let isEditMode = false; // Flag to track whether we're in edit mode
        let jobId = null; // To store the country ID for updating
        ...
        $('#salary_range_hidden').val(salaryValues[0] + '-' + salaryValues[1]);
            ...
            const gender = $('input[name="gender"]:checked').val(); // For radio buttons

            formData.append('gender', gender);
            // const gender = $('input[name="gender"]:checked').val();
            // const formData = $(this).serialize() + "&gender=" + gender; 

            // Clear previous validation messages

            const url = isEditMode ? `/api/job/${jobId}` : '/api/job'; // Endpoint for update or create
                ...
                error: function(xhr) {
                    ...
                    $.each(errors, function(key, value) {
                        ...
                        if (key === "gender") {
                            ...
                            // Append error message below gender radio buttons group
                            $('.gender-group').append(`<div class="text-danger gender-error">${value}</div>`);

                            // Add is-invalid class to all gender radio buttons
