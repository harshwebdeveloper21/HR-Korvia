            ...
            <div class="card custom-card rounded-3 shadow-sm">
                <div class="card-header d-flex align-items-center gap-2 py-3 px-4">
                    <span style="background-color:#E66136; color:#fff; border-radius:8px; width:36px; height:36px; display:flex; align-items:center; justify-content:center;">
                        <i class="mdi mdi-pencil-box-outline fs-18"></i>
                    </span>
                    <div class="card-title fw-bold mb-0" style="font-size: 1.1rem;">Edit Announcement</div>
                </div>
                <div class="card-body p-4">
                    <form id="editAnnouncementForm" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="row gy-4">
                            ...
                            <!-- Specific Role Selection (Pill Style) -->
                                ...
                                <div class="input-group align-items-stretch">
                                    ...
                                    <div class="form-control d-flex flex-wrap align-items-center gap-3" style="height:auto; min-height:50px; background:#fff;">
                                        <label class="role-pill-label" for="roleAdmin">
                                            <input type="checkbox" class="role-pill-input" name="target_roles[]" value="admin" id="roleAdmin" <?= in_array('admin', $targetRoles) ? 'checked' : '' ?>>
                                            <span class="role-pill"><i class="mdi mdi-shield-crown-outline me-1"></i>Admin</span>
                                        </label>
                                        <label class="role-pill-label" for="roleHr">
                                            <input type="checkbox" class="role-pill-input" name="target_roles[]" value="hr" id="roleHr" <?= in_array('hr', $targetRoles) ? 'checked' : '' ?>>
                                            <span class="role-pill"><i class="mdi mdi-account-tie-outline me-1"></i>HR</span>
                                        </label>
                                        <label class="role-pill-label" for="roleEmployee">
                                            <input type="checkbox" class="role-pill-input" name="target_roles[]" value="employee" id="roleEmployee" <?= in_array('employee', $targetRoles) ? 'checked' : '' ?>>
                            ...
                            <!-- Specific User Selection (Select2) -->
                                    ...
                                    <div class="flex-grow-1">
                                        <select class="form-control select2-users" id="target_users" name="target_users[]" multiple>
                                            <?php foreach ($users as $u): ?>
                                                <option value="<?= $u['id'] ?>" <?= in_array($u['id'], $targetUsers) ? 'selected' : '' ?>><?= esc($u['username']) ?> (<?= ucfirst($u['role']) ?>)</option>
                                            <?php endforeach; ?>
                                        </select>
                                    </div>
                                </div>
                            ...
                            <div class="col-xl-12">
                             ...
                             <div class="col-xl-12 d-flex justify-content-end gap-2 mt-4 pt-3 border-top">
                                 ...
                                 <button type="submit" id="editSubmitBtn" class="btn px-4 text-white fw-semibold" style="background-color: #E66136; border-radius: 8px; box-shadow: 0 4px 12px rgba(230, 97, 54, 0.2);">
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.status === 'success') {
                    ...
                    $btn.prop('disabled', false).html('<i class="mdi mdi-content-save me-1"></i>Save Changes');
