        ...
        <div class="card">
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-center mb-3">
                    <h4 class="card-title">All EOM Templates</h4>

                    <a href="/add-empof-month" class="btn hr-btnbg attendenceall">
...
<script>
    function loadTemplates() {
        $.ajax({
            url: '<?= site_url("api/empof-month/list") ?>',
            type: 'GET',
                ...
                if (response.status === 'success' && response.data.length > 0) {
                    ...
                    response.data.forEach((template, index) => {
                        html += `
                            <tr>
                                <td>${index + 1}</td>
                                <td class="capitalize-text">${template.title}</td>
                                <td style="display: flex; align-items: center; gap: 8px;">
                                    <a href="/empof_month/view/${template.id}" class="text-primary fs-5" title="View"><i class="mdi mdi-eye"></i></a>
                                    <a href="/empof_month/${template.id}" class="text-warning fs-5" title="Edit"><i class="mdi mdi-pencil"></i></a>
